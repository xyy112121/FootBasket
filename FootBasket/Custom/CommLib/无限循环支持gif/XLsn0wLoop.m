...
- (void)setPagePosition:(PageControlPosition)pagePosition {
	...
	_pageControl.frame = CGRectMake(_pageControl.frame.origin.x, _pageControl.frame.origin.y-5, _pageControl.frame.size.width,_pageControl.frame.size.height);
	DLog(@"pagecontrol===%f,%f,%f,%f",_pageControl.frame.origin.x,_pageControl.frame.origin.y,_pageControl.frame.size.width,_pageControl.frame.size.height);
}
...
- (void)imageClick {
    ...
    } else if ([self.xlsn0wDelegate respondsToSelector:@selector(loopView:clickImageAtIndex:)]){
        [self.xlsn0wDelegate loopView:self clickImageAtIndex:self.currIndex];
    }
}
...
- (void)downloadImages:(int)index {
	...
	{
		subURLString = [urlString substringFromIndex:10];
	}
    else
	{
		subURLString = @" ";
