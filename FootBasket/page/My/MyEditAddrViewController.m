...
@interface MyEditAddrViewController ()

@end

@implementation MyEditAddrViewController
...
-(void)initview
{
    ...
    FCname = [_FCdicaddr objectForKey:@"userName"];
    FCtel = [_FCdicaddr objectForKey:@"mobile"];
    FCprovice = [_FCdicaddr objectForKey:@"province"];
    FCcity = [_FCdicaddr objectForKey:@"city"];
    FCarea = [_FCdicaddr objectForKey:@"county"];
    FCdetailaddr = [_FCdicaddr objectForKey:@"address"];
    if([[_FCdicaddr objectForKey:@"isDefault"] intValue]==1)
    {
...
-(void)clickdone:(id)sender
{
    ...
    else
    {
        ...
        [addrservice sendUpdateAddressRequest:FCprovice City:FCcity Area:FCarea Address:FCdetailaddr Tel:FCtel IsDefault:selectresult==EnSelected?@"1":@"0" UserName:FCname ObjectId:[_FCdicaddr objectForKey:@"id"] App:app ReqUrl:RQMyUpdateAddr successBlock:^(NSDictionary *dicData)
        {
            [self returnback:nil];
        }];
    }
    ...
    if(indexPath.section == 0)
    {
        ...
        switch (indexPath.row)
        {
            ...
            case 2:
                ...
                textfieldvalue.text = [NSString stringWithFormat:@"%@%@%@",FCprovice,FCcity,FCarea];
                break;
            case 3:
                labelname.text = @"详细地址";
